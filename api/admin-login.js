...
function safeEqual(a, b) { return typeof a === 'string' && typeof b === 'string' && a.length === b.length && crypto.timingSafeEqual(Buffer.from(a), Buffer.from(b)); }
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const valid = !!value && !!signature && safeEqual(signature, sign(value, secret));
    return res.status(valid ? 200 : 401).json({ authenticated: valid });
  }

  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });
  const body = typeof req.body === 'string' ? JSON.parse(req.body) : (req.body || {});
  if (!safeEqual(body.password, password)) return res.status(401).json({ error: 'Incorrect password.' });
